...
const DEFAULT_ENCODING = "big5";
const STEEL_ASSEMBLY = "SteelAssembly";
const STEEL_PART = "SteelPart";
const STEEL_BOLTS = "SteelBolts";
var BoltType;
(function (BoltType) {
  BoltType["BOLT"] = "BOLT";
  BoltType["STUD"] = "STUD";
})(BoltType || (BoltType = {}));
class Part extends BomItem {
  constructor(project, data) {
    var _data$nc;
    super(project, data.id);
    this.name = data.name;
    ...
    this.nc = data.nc;
    this.bolts =
      ((_data$nc = data.nc) === null || _data$nc === void 0
        ? void 0
        : _data$nc.bo) ?? [];
  }
  get materialType() {
  ...
  specSameAs(item) {
    return (
      ...
      this.area === item.area &&
      this.drawingName === item.drawingName &&
      this.nc === item.nc
    );
  }
  toString() {
    return `${this.materialType}::${this.name}`;
  }
}
class Assembly extends BomItem {
  constructor(project, data) {
    super(project, data.id);
    this.partCounts = {};
    this.boltCounts = {};
    this.studCounts = {};
    this.name = data.name;
    this.installPosition = data.installPosition;
    this.installHeight = data.installHeight;
    this.areaType = data.areaType;
    this.transportNumber = data.transportNumber;
    this.transportDesc = data.transportDesc;
    this.drawingName = data.drawingName;
    ...
    this.bolts = [];
    this.studs = [];
    this.mainPart = new Part(project, {
      id: data.mainPartId,
      name: data.mainPartName,
      specification: data.mainPartSpecification,
      type: data.mainPartType,
      length: data.mainPartLength,
      height: data.mainPartHeight,
      width: data.mainPartWidth,
      t1: data.mainPartT1,
      t2: data.mainPartT2,
      material: data.mainPartMaterial,
      weight: data.mainPartWeight,
      area: data.mainPartArea,
      drawingName: data.mainPartDrawingName,
    });
  }
  get materialType() {
  ...
  addPart(part) {
    ...
    this.partCounts[part.name] = (this.partCounts[part.name] ?? 0) + 1;
  }
  addBolt(bolt) {
    const quantity = Number(bolt.quantity);
    if (bolt.type === BoltType.BOLT) {
      this.bolts.push(bolt);
      this.boltCounts[bolt.name] = (this.partCounts[bolt.name] ?? 0) + quantity;
    } else if (bolt.type === BoltType.STUD) {
      this.studs.push(bolt);
      this.studCounts[bolt.name] = (this.studCounts[bolt.name] ?? 0) + quantity;
    }
  }
  specSameAs(item) {
    if (
      ...
      this.transportNumber !== item.transportNumber ||
      this.transportDesc !== item.transportDesc ||
      this.drawingName !== item.drawingName ||
      this.totalWidth !== item.totalWidth ||
      this.totalHeight !== item.totalHeight ||
    ...
    for (const [pName, count] of Object.entries(this.partCounts)) {
      if (item.partCounts[pName] !== count) {
        return false;
      }
    }
    for (const [bName, count] of Object.entries(this.boltCounts)) {
      if (item.boltCounts[bName] !== count) {
        return false;
      }
    }
    for (const [sName, count] of Object.entries(this.studCounts)) {
      if (item.studCounts[sName] !== count) {
        return false;
      }
    }
    return true;
...
function csvColumnExtract(row, options) {
  const separator =
  ...
  const columnNames =
    options === null || options === void 0 ? void 0 : options.columnNames;
  const withColumnNameOnly =
    (options === null || options === void 0
      ? void 0
      : options.withColumnNameOnly) ?? true;
  const result = {};
  for (let i = 0; i < cols.length; i++) {
    const fieldName =
      columnNames === null || columnNames === void 0 ? void 0 : columnNames[i];
    if (!fieldName && withColumnNameOnly) {
      continue;
    }
    result[fieldName ?? i] = cols[i].trim();
  }
  return result;
}
class AssemblyParser {
  parse(text, project, pos) {
    const spec = csvColumnExtract(text, {
      columnNames: [
        undefined,
        "id",
        "name",
    ...
    return new Assembly(project, spec);
  }
}
class NcConflictException extends Error {
  constructor(item, pos) {
    super(
      `BOM 零件規格與 NC 不同, id:${item.id} (${item.toString()}) 在 ${pos}`
    );
  }
}
class ParseFailedException extends Error {
  constructor(message) {
    super(message);
  }
}
class PartParser {
  parse(text, project, pos) {
    ...
    if (isNaN(Number(spec.length))) {
      throw new ParseFailedException(`'${spec.length}' 不是合法的長度`);
    }
    const nc = project.getNC(spec.name);
    const part = new Part(project, {
      ...spec,
      nc,
    });
    if (
      nc &&
      !project.options.ignoreNcSpecNotMatch &&
      this.isNcConflict(part, nc)
    ) {
      throw new NcConflictException(part, pos);
    }
    return part;
  }
  isNcConflict(part, nc) {
    const ncLenDiff = Number(nc.length) - Number(part.length);
    // 避免福點數精確度影響
    if (Math.abs(ncLenDiff) > 1.000000001) {
      return true;
    }
    return (
      part.name !== nc.name ||
      part.specification !== nc.specification ||
      part.material !== nc.material
    );
  }
}
class BoltParser {
...
function generateNcBoId() {
  let id;
  while (existIds.has((id = randomString()))) {} // 避免 ID 重複
  existIds.add(id);
  return id;
}
...
class NcParser {
  parse(text, project, pos) {
    const spec = this.parseNcRawText(text);
    if (
      spec.name === undefined ||
      spec.specification === undefined ||
      isNaN(Number(spec.length)) ||
      spec.material === undefined
    ) {
      throw new ParseFailedException(`無法正常取得資料: ${pos}`);
    }
    return new NC({
      name: spec.name,
  ...
  parseNcRawText(text) {
    ...
    for (; rowIdx < rows.length; rowIdx++) {
      const row = rows[rowIdx].trimEnd();
      if (row === "EN") {
        break;
      }
      if (row.startsWith("** ")) {
        scopeStack.push("**"); // into ** scope
        spec.name = rows[(rowIdx += 3)].trim();
        spec.material = rows[(rowIdx += 2)].trim();
        spec.specification = rows[(rowIdx += 2)].trim();
        continue;
      }
      const scope = scopeStack.at(-1);
      if (scope === "**") {
        if (row === "" && spec.body.length === 0) {
          //空行
          spec.body = rows.slice(rowIdx).join("\n").trim();
        } else if (row.match(/^\s+I$/)) {
          // I
          spec.length = rows[(rowIdx += 1)].trim();
        } else if (row === "BO") {
          scopeStack.push("BO");
        }
        continue;
      ...
      if (scope === "BO") {
        spec.bo.push(this.parseBO(row));
      }
    }
    return spec;
  }
  parseBO(row) {
    const [facing, x, y, z] = row
      .trim()
      .split(/\s+/)
      .map((s) => s.trim());
    const id = generateNcBoId();
    const type = "BO";
    return {
      id,
      type,
...
const boltParser = new BoltParser();
const ncParser = new NcParser();
class Project {
  constructor(bomFile, ncFiles, options) {
    this._options = options;
    this.bomFile = bomFile.filepath;
    this.bomContent = bomFile.content;
  ...
  static mergeDefaultOptions(options) {
    return {
      ...
      ignoreNcSpecNotMatch: false,
      ignoreSpecConflict: false,
      ...options,
    };
  }
  static async readNcDir(dir) {
    const list = await fs.readdir(dir);
    return list
      .filter((p) => p.endsWith(".nc1"))
      .map((p) => path.resolve(dir, p));
  }
  static async fromFiles(bomFilePath, ncFilePaths, options) {
    const finalOptions = Project.mergeDefaultOptions(options);
    const bomContent = libExports.decode(
      await fs.readFile(bomFilePath),
      finalOptions.encoding
    );
    const ncFiles = await Promise.all(
      ncFilePaths.map(async (p) => {
        const pathInfo = path.parse(p);
        const name = pathInfo.name;
        const content = libExports.decode(
          await fs.readFile(p),
          finalOptions.encoding
        );
        return {
  ...
  static async fromBomFileAndNcDir(bomFilePath, ncDir, options) {
    return this.fromFiles(
      path.resolve(bomFilePath),
      await this.readNcDir(ncDir),
      options
    );
  }
  static async fromProjectDir(dir, options) {
    const bomFilePath = path.resolve(dir, "TeklaBom.csv");
    const ncDir = path.resolve(dir, "DSTV_Profile");
    return this.fromBomFileAndNcDir(bomFilePath, ncDir, options);
  }
  static fromJson(json, options) {
    const { bom, nc } = JSON.parse(json);
    const ncFiles = nc.map((n) => ({
      name: n.filename,
      fullName: n.filename,
      content: n.content,
    }));
    return new Project(
      {
        filepath: bom.filename,
        content: bom.content,
      },
      ncFiles,
      Project.mergeDefaultOptions(options)
    );
  }
  get options() {
  ...
  addAssembly(assembly) {
    this.items.assemblies.push(assembly);
    if (!this.specs.assemblies.hasOwnProperty(assembly.name)) {
      this.specs.assemblies[assembly.name] = assembly;
    }
  }
  addPart(part) {
    this.items.parts.push(part);
    if (!this.specs.parts.hasOwnProperty(part.name)) {
      this.specs.parts[part.name] = part;
    }
  }
  addBolt(bolt) {
    if (bolt.type === BoltType.BOLT) {
      if (!this.specs.bolts.hasOwnProperty(bolt.name)) {
        this.items.bolts.push(bolt);
        this.specs.bolts[bolt.name] = bolt;
      }
    } else if (bolt.type === BoltType.STUD) {
      if (!this.specs.studs.hasOwnProperty(bolt.name)) {
        this.items.studs.push(bolt);
        this.specs.studs[bolt.name] = bolt;
      }
    }
  }
  getSpec(item) {
    if (item instanceof Assembly) {
      return this.getAssemblySpec(item.name);
    } else if (item instanceof Part) {
      return this.getPartSpec(item.name);
    } else if (item instanceof Bolt) {
      return this.getBoltSpec(item.type, item.name);
    }
  }
  getAssemblySpec(name) {
    return this.specs.assemblies[name];
  }
  getPartSpec(name) {
    return this.specs.parts[name];
  }
  getBoltSpec(type, name) {
    return type === BoltType.BOLT
      ? this.specs.bolts[name]
      : type === BoltType.STUD
        ? this.specs.studs[name]
        : undefined;
  }
  getNC(name) {
    let nc = this.specs.nc[name];
    if (nc) {
      return nc;
    }
    const ncFile = this.ncFiles.find((f) => f.name === name);
    if (!ncFile) {
      return undefined;
    }
    nc = ncParser.parse(ncFile.content, this, ncFile.fullName);
    this.specs.nc[name] = nc;
    return nc;
  }
}
class SpecConflictException extends Error {
  constructor(item, pos) {
    super(
      `存在相同編號的不同規格, id:${item.id} (${item.toString()}) 在 ${pos}`
    );
  }
}
function generateResult(project) {
  const assemblyTemplates = generateAssemblyTemplates(
    Object.values(project.specs.assemblies)
  );
  const partTemplates = generatePartTemplates(
    Object.values(project.specs.parts)
  );
  const boltTemplates = generateBoltTemplates(
    Object.values(project.specs.bolts)
  );
  const studTemplates = generateStudTemplates(
    Object.values(project.specs.studs)
  );
  const root = generateRoot(project);
  return {
    version: APP_VERSION,
...
function generateAssemblyTemplates(assemblies) {
  return assemblies.map((assembly) => ({
    name: assembly.name,
    drawingName: assembly.drawingName,
    totalWidth: Number(assembly.totalWidth),
    totalHeight: Number(assembly.totalHeight),
    totalLength: Number(assembly.totalLength),
    totalWeight: Number(assembly.totalWeight),
    totalArea: Number(assembly.totalArea),
    partTemplates: Object.entries(assembly.partCounts).map(([name, count]) => ({
      name,
      count,
    })),
    boltTemplates: Object.entries(assembly.boltCounts).map(([name, count]) => ({
      name,
      count,
    })),
    studTemplates: Object.entries(assembly.studCounts).map(([name, count]) => ({
      name,
      count,
    })),
    customTemplates: [],
...
function generatePartTemplates(parts) {
  return parts.map((part) => {
    ...
    return {
      ...
      nc:
        (_part$nc = part.nc) === null || _part$nc === void 0
          ? void 0
          : _part$nc.body,
      bolts: part.bolts.map((b) => ({
        id: b.id,
        type: b.type,
        facing: b.facing,
        x: b.x,
...
function generateBoltTemplates(bolts) {
  return bolts.map((bolt) => ({
    name: bolt.name,
    type: bolt.type,
    material: bolt.material,
    weight: Number(bolt.weight),
  }));
}
function generateStudTemplates(bolts) {
  return bolts.map((stud) => ({
    name: stud.name,
    type: stud.type,
    material: stud.material,
    weight: Number(stud.weight),
  }));
}
function generateRoot(project) {
  const assemblies = project.items.assemblies.map((a) => ({
    id: a.id,
    name: a.name,
    installPosition: a.installPosition,
    installHeight: Number(a.installHeight),
    areaType: a.areaType,
    transportNumber: a.transportNumber,
    transportDesc: a.transportDesc,
    memo: a.memo,
    mainPart: {
      specification: a.mainPart.specification,
      material: a.mainPart.material,
      type: a.mainPart.type,
    },
    parts: a.parts.map((p) => ({
      id: p.id,
      name: p.name,
...
function transform(project) {
  ...
  return generateResult(project);
}
function parseProject(project) {
  const bom = project.bomContent;
  const rows = bom.split(/\r?\n/);
  let currentAssembly = undefined;
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    const lineNumber = i + 1;
    const pos = `${project.bomFile}:${lineNumber}`;
    const type = csvColumnExtract(row, {
      columnNames: ["type"],
    }).type;
    // SteelAssembly
    if (type === STEEL_ASSEMBLY) {
      if (currentAssembly) {
        if (isSpecConflict(project, currentAssembly)) {
          throw new SpecConflictException(currentAssembly, pos);
        }
      }
      currentAssembly = assemblyParser.parse(row, project, pos);
      project.addAssembly(currentAssembly);
      continue;
    }
    if (!currentAssembly) {
      continue;
    ...
    // SteelPart
    if (type === STEEL_PART) {
      const part = partParser.parse(row, project, pos);
      if (isSpecConflict(project, part)) {
        throw new SpecConflictException(part, pos);
      }
      currentAssembly.addPart(part);
      project.addPart(part);
    }
    // SteelBolts
    else if (type === STEEL_BOLTS) {
      const bolt = boltParser.parse(row, project, pos);
      if (isSpecConflict(project, bolt)) {
        throw new SpecConflictException(bolt, pos);
      }
      currentAssembly.addBolt(bolt);
      project.addBolt(bolt);
    }
  }
}
function isSpecConflict(project, item) {
  if (!project.options.ignoreSpecConflict) {
    const spec = project.getSpec(item);
    return spec && !spec.specSameAs(item);
  }
}

export { Project, transform };
export default { Project, transform };
...
//     Project,
//     transform,
//     // Any other things you want in the default export
//   },
// };
