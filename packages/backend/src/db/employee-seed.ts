    ...
    for (let i = 0; i < dept.count; i++) {
      ...
      const emps = await db
        ...
        .values({
          ...
          enName: `John Doe ${i + 1}`,
          birthday: new Date(
            Math.floor(Math.random() * 40 * 365 * 24 * 60 * 60 * 1000) +
              946684800000
          ),
          ...
          residenceCounty: ["台北市", "新北市", "桃園市"][
            Math.floor(Math.random() * 3)
          ],
          residenceDistrict: ["中正區", "信義區", "大安區"][
            Math.floor(Math.random() * 3)
          ],
          residenceAddress: `台北市中正區${Math.floor(Math.random() * 100)}號`,
          mailingCounty: ["台北市", "新北市", "桃園市"][
            Math.floor(Math.random() * 3)
          ],
          mailingDistrict: ["中正區", "信義區", "大安區"][
            Math.floor(Math.random() * 3)
          ],
